...
The Azure Speech Key configured below is required. It is used for pronunciation assessment and for generating reference audio in some modes. In Repeat Sentence, for example, you need to hear the prompt before repeating it; without that audio, the exercise would not be authentic. Every API call may count toward Azure usage. In my own testing I stayed within the free allowance and did not pay anything, but I still dislike making a paid request every time the same fixed audio is needed. The app therefore caches Repeat Sentence, Read Aloud, and word/phrase reference audio locally by a hash of the text, avoiding duplicate Azure TTS requests.

Across all three pages, `Space` is the submit shortcut. It uploads the current recording to Azure and returns an assessment result. The arrow keys are also used frequently: `Up` moves to the previous item, `Down` moves to the next, and `Left` and `Right` have page-specific actions.
...
In this project, Repeat Sentence and Read Aloud are treated as one practice flow. Both tasks involve speaking a fixed sentence: one sentence is heard first, while the other is read from text. The app therefore combines them into the same page. The tool began as something simple, but as I used it I kept discovering another feature I wanted, and then another, until the page became much more capable. The guiding principle throughout was practical usefulness.
...
The screenshot below shows the complete information produced after I read the first sentence. I tried to expose as much useful feedback as possible. It is difficult to predict which metrics will matter to each learner, and having more information is better than having none.
...
That said, nobody outside the official PTE system knows exactly how the real scoring backend works. These results should be treated as training references rather than a perfect replica of the exam. In practice, despite the volume and technical depth of the feedback, I mostly use the tool to find weak words and adjust my pitch and rhythm. Even after understanding a pronunciation problem intellectually, retraining the mouth's muscle memory to produce the correct sound remains a substantial challenge.
...
In practice, the text comparison is often of limited value—sometimes even less useful than the comparison in my writing-email tool. In writing, it can reveal words that are repeatedly omitted or misspelled. Speaking includes spontaneous phrasing and intense time pressure, so I have gained less from exact text comparison alone. It can still expose important problems, especially when I swallow the beginning or ending of a sentence and Azure objectively recognizes something completely different from what I thought I said.
